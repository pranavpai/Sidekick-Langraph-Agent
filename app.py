...
async def generate_clarifying_questions(sidekick, message, success_criteria, chatbot):
    try:
        ...
        return [f"Error generating questions: {str(e)}", "", ""], gr.update(visible=True), gr.update(visible=False)
...
async def process_with_clarifying(sidekick, message, success_criteria, chatbot, q1_answer, q2_answer, q3_answer, clarifying_questions):
    ...
    try:
        ...
        print(f"⏱️ [CLARIFYING] Starting run_superstep with 120s timeout protection...")
        try:
            ...
            print(f"⏰ [CLARIFYING] run_superstep timed out after 120s, falling back...")
        ...
        print(f"🔄 [CLARIFYING] Attempting fallback to direct processing...")
        try:
...
async def process_message_direct(sidekick, message, success_criteria, chatbot):
    ...
    try:
        ...
        # Return error state
        error_history = chatbot + [{"role": "assistant", "content": f"Error in direct processing: {str(e)}"}]
